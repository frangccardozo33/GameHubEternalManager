# Circuitos de la Serie Nacional GT3 — UN SOLO PROMPT

Encargo único para una IA de código/geometría: **rehacer los 20 circuitos** (trazado + decorado) en un solo pedido. Incluye las reglas técnicas del juego,
la ficha de cada circuito (con el lore nuevo y los países del Continente Viejo: Baikal, Estovackia, Kostanay, Netanya y Overmark) y el formato de entrega.
Es autosuficiente: la IA no necesita conocer el proyecto.

## Cómo usarlo
1. Copiá **todo** el bloque de la sección «PROMPT» y pegalo en la IA.
2. Si la respuesta se corta o no entran los 20 en un mensaje, escribile: **«Continuá con los siguientes 5 circuitos, mismo formato»**. Cada mensaje debe ser un objeto JSON válido.
3. Guardá lo que devuelva como `07-carreras-apex/circuits/circuits.json` (un solo archivo con los 20) o, si vino en partes, juntalas con
   `node 07-carreras-apex/tools/merge_bundle.mjs parte1.json parte2.json …` (crea `circuits/circuits.json`).
4. Verificá: `node 07-carreras-apex/tools/check_circuit.mjs 07-carreras-apex/circuits/circuits.json` (cada circuito debe dar ✓; los ERROR se le pasan a la IA para que los corrija).
5. Probalo en el juego: `python -m http.server 8080` → `http://localhost:8080/07-carreras-apex/index.html` → Campeonato → Atlas de circuitos → elegir → «USAR EN ESTA FECHA».
   En la consola: `await CIRCUITS_READY` (cantidad de circuitos cargados; los rechazados avisan con `[circuits]`).
Un circuito suelto también se puede poner como `circuits/<id>.json` (pisa al del bundle). **Modelo de ejemplo ya armado:** `07-carreras-apex/circuits/_modelo_valleverde.json`
(o el botón «⬇ Descargar JSON del circuito actual» del Atlas de circuitos del juego).

## PROMPT
```
Sos un diseñador de pistas para un videojuego de carreras de autos GT3 (motor three.js, mundo ficticio). Vas a rehacer los 20 circuitos de la temporada.
Devolvé UN SOLO objeto JSON válido, sin comentarios ni texto alrededor, con una clave por id de circuito:

{ "valleverde": { ... }, "autodromocentral": { ... }, ... }   // las 20 claves, en el orden de las fichas

Cada circuito tiene esta forma (todo lo demás se ignora):
{
  "id": "<id>", "theme": "grass|desert|coast|forest|city|night|mountain", "halfWidth": 8.5,
  "groundColor": "#rrggbb", "skyColor": "#rrggbb", "foliageColor": "#rrggbb",     // opcionales
  "treeCount": 0-320, "hills": true|false,                                          // opcionales (árboles y colinas automáticos)
  "gripMod": 0.7-1.3, "brakingMod": 0.7-1.4, "aeroMod": 0.7-1.4, "overtakeDiff": 0.1-0.95, "wetChance": 0-0.8, "tempBase": -10..45,   // usá los de la ficha
  "layout": [[x,z], ...],                     // 8-60 puntos, spline cerrada
  "props": [ ... entre 120 y 300 objetos ... ]
}

REQUISITOS TÉCNICOS (obligatorios; el juego rechaza el circuito si no se cumplen):
...
CONTENIDO CREATIVO
- Hacé que los 20 circuitos se sientan MUY distintos entre sí: forma del trazado (óvalo, tri-óvalo, horquillas, eses, calle en cuadrícula, ribera, cráter), largo (1,6 a 3,2 km según la ficha), ancho, paleta de colores y decorado.
- Respetá de cada ficha: el tipo, el largo objetivo, la cantidad de curvas, que las curvas con nombre existan en ese orden de vuelta con el carácter descrito (lenta/rápida, ciega, horquilla, chicane, S),
  el sentido de giro y la silueta, el paisaje y el kit de decorado, los colores y los números de juego.
- Armá escenas reconocibles con las primitivas (un faro = cylinder + cone; una grúa = box altas + box horizontal; un cohete = cylinder + cone; una cabaña = box + cone). No esparzas props al azar.
- Los circuitos con el ícono «CONTINENTE VIEJO» pertenecen a naciones nuevas del mundo; su decorado tiene que ser inconfundible (hielo, estepa con cohetes, salar, fiordo, ciudad industrial).

AUTOVERIFICACIÓN (hacela antes de responder, circuito por circuito)
1) layout[0..2] sobre z = 135 con x creciente; último punto sobre z = 135; recta recta de x = −190 a 85; nada de otro tramo en x −200…110, z 70…200.
2) Sin cruces; ≥ 60 m entre tramos no consecutivos; ningún par de puntos a menos de 25 m; radio mínimo ≥ 25 m (nunca < halfWidth + 2,5); largo 1,6–3,2 km.
3) Ningún prop a menos de 22 m del eje de la pista ni en la franja z 84…132 de la recta principal; ≤ 300 props; colores en #rrggbb; sign.text ≤ 18 caracteres.
4) halfWidth ≤ 9,5. Los tres primeros puntos exactamente `[-220,135], [-80,135], [100,135]` salvo que necesites una recta más larga (entonces x más grande, siempre z = 135).
...
## Notas para quien revisa el resultado
- Si el validador marca «radio mínimo» como aviso (no error) el circuito funciona pero tiene horquillas de ~40 km/h; si marca ERROR el juego lo mostraría roto (asfalto plegado, boxes tapados o cruces).
- `zenet` (Zenet) no tiene fecha esta temporada: sus 20 fechas van a las otras naciones. `lagunaazul` (antes «Laguna Azul», Zenet) es ahora **MAR SALADO** (Netanya); los ids de archivo no cambian.
- Los nombres visibles de circuito se definen en `07-carreras-apex/circuits.js` (`TRACK_NAMES` y `CIRCUIT_META`); la IA no tiene que tocarlos.
